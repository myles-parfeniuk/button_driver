...
\begin{DoxyCompactList}\small\item\em Data\+Wrapper containing button event, call-\/back function can be registered to button with follow() method (see data\+\_\+control documentation) \end{DoxyCompactList}\item 
\label{class_button_a5a60233721ae6d1ba8799be7d9f72679} 
Task\+Handle\+\_\+t {\bfseries button\+\_\+task\+\_\+hdl}
\begin{DoxyCompactList}\small\item\em task handle of the button\+\_\+task \end{DoxyCompactList}\end{DoxyCompactItemize}
...
\begin{DoxyCompactList}\small\item\em Task responsible for determining and generating events. \end{DoxyCompactList}\end{DoxyCompactItemize}
\doxysubsubsection*{Static Private Member Functions}
\begin{DoxyCompactItemize}
\item 
static void \textbf{ button\+\_\+task\+\_\+trampoline} (void $\ast$arg)
...
\begin{DoxyCompactList}\small\item\em name of button, used in debugging logs \end{DoxyCompactList}\end{DoxyCompactItemize}
...
Call the follow() method on the event member of a button to register a call-\/back function (see data\+\_\+control documentation for more info on follow()).
...
Any call-\/back functions registered to a button\textquotesingle{}s event member will be executed every time a button event is detected. ~\newline
 Call-\/back functions can be registered in as many places as desired using follow(), where each button event can be handled in multiple places, independently.
...
This ISR is entered when a falling edge is detected (for an active low button) or a rising edge (for an active high button). It resumes the button\+\_\+task and disables the button\textquotesingle{}s interrupt.

\doxyref{Button}{p.}{class_button} interrupt is re-\/enabled in button\+\_\+task upon button release.

button activity triggers isr -\/-\/$>$ interrupt disabled -\/-\/$>$ button\+\_\+task notified -\/-\/$>$ events are determined and generated -\/-\/$>$ button release detected -\/-\/$>$ interrupt enabled -\/-\/$>$ button\+\_\+task waits for notification
...
Task responsible for determining and generating events. 

This task is notified by the button\+\_\+handler ISR when any button activity is detected. It determines the correct button events and generates them until the button is released. After the button is released the task waits until it is again notified by the button\+\_\+handler ISR.
...
void, nothing to return 
\end{DoxyReturn}
\label{class_button_aae67a1e052c81ad02d1e0ecc8e823af5} 
...
\doxysubsubsection{button\_task\_trampoline()}
{\footnotesize\ttfamily void Button\+::button\+\_\+task\+\_\+trampoline (\begin{DoxyParamCaption}\item[{void $\ast$}]{arg }\end{DoxyParamCaption})\hspace{0.3cm}{\ttfamily [static]}, {\ttfamily [private]}}



Launches button task. 

This function is used to get around the fact x\+Task\+Create() from the freertos api requires a static task function.

To prevent having to write the button task from the context of a static function, this launches the \doxyref{button\+\_\+task()}{p.}{class_button_ac7e3de5e746361d1a4e6aa6a5bbcb793} from the \doxyref{Button}{p.}{class_button} object passed into x\+Task\+Create().
...
\begin{DoxyReturn}{Returns}
True if button was released (quick-\/press event generated), false if button was held for until long\+\_\+press\+\_\+evt\+\_\+time elapsed. 
\end{DoxyReturn}
...
Instead of calling \doxyref{scan()}{p.}{class_button_a713b68291767522afcc497527010583f}, it is recommended call the follow() method on the event member to register a call-\/back function.
...
\item 
D\+:/development/git/esp\+\_\+\+BNO08x\+\_\+testing/components/button\+\_\+driver/Button.\+hpp\item 
D\+:/development/git/esp\+\_\+\+BNO08x\+\_\+testing/components/button\+\_\+driver/Button.\+cpp\end{DoxyCompactItemize}
